...
static inline void _specialized_memcpy(void *dest, const void *src, size_t n) {
#define _specialize_type(type) {            \
    if (n == sizeof(type)) {                \
    ...
    _specialize_type(uint8_t);
    _specialize_type(uint16_t);
    _specialize_type(uint32_t);
    _specialize_type(uint64_t);
#if __SIZEOF_INT128__
    _specialize_type(unsigned __int128);
#endif

    memcpy(dest, src, n);
}

static inline void _swap(char *a, char *b, size_t obj_size) {
    if (a == b) return;
...
static inline void _heap_sort_heapify(char *array,
    ...
    while (i * 2 + 1 < n) {
        ...
        if (compare_func(array + larger_child * obj_size, array + i * obj_size) > 0) {
            _swap(array + larger_child * obj_size, array + i * obj_size, obj_size);
            i = larger_child;
        } else break;
    }
}
...
static inline void _heap_sort(char *array,
    ...
    for (size_t i = (n - 1) / 2; i < n; i--) {
        _heap_sort_heapify(array, i, n, obj_size, compare_func);
    }
    ...
    for (size_t m = n; m > 1; m--) {
        ...
        _swap(array + 0 * obj_size, array + (m - 1) * obj_size, obj_size);

        // Maintain the heap
        _heap_sort_heapify(array, 0, m - 1, obj_size, compare_func);
    }
}
...
static inline void _quick_sort_partition(char *array,
    ...
    _swap(array + 0 * obj_size, pivot, obj_size);
...
#define _stack_size()           (stack_top - (stack_bottom - 1))
...
#define _next_state_from_stack() { \
    ...
    while (1) {
        ...
        if (n <= INSERTION_SORT_COUNT_LIMIT) {
            _insertion_sort(array, n, obj_size, compare_func);
            _next_state_from_stack();
        }

        char *pivot = _quick_sort_choose_pivot(array, n, obj_size, compare_func);

        char *pivot_l, *pivot_r;
        _quick_sort_partition(array, n, obj_size, compare_func, pivot,
                              &pivot_l, &pivot_r);
        ...
        if (_stack_size() == max_depth) {
            if (left_n > 1) _heap_sort(array_l, left_n, obj_size, compare_func);
            if (right_n > 1) _heap_sort(array_r, right_n, obj_size, compare_func);
            _next_state_from_stack();
        }
        ...
        if (left_n > 1) {
            ...
            if (right_n > INSERTION_SORT_COUNT_LIMIT)
                ...
                _insertion_sort(array_r, right_n, obj_size, compare_func);
...
void _qsort(void *base,
    ...
    _quick_sort(base, n, obj_size, compare_func, max_depth);
}
